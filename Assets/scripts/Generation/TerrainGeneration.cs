...
public class TerrainGeneration : MonoBehaviour
{
    ...
    private void GameBoardGeneration(Tableau toInstanciate)
    {
            ...
            for (int j = 0; j < toInstanciate.matrixLength; j++)
            {
                for (int k = 0; k < 10; k++)
                {
                    for (int l = 0; l < 10; l++)
                    {
                        ...
                        Vector3 blocPos = new Vector3(j*distanceBtwRooms + l, 0, -i*distanceBtwRooms - k);
        ...
        {
            Vector3 blocPos1 = new Vector3(-1, 0, -i);
            transform.position = blocPos1;
            InstanciateWall();

            Vector3 blocPos3 = new Vector3(toInstanciate.matrixLength * 10, 0, -i);
            transform.position = blocPos3;
            InstanciateWall();
        }
            
        for (int j = -1; j < toInstanciate.matrixLength*10+1; j++)
        {
            Vector3 blocPos1 = new Vector3(j, 0, 1);
            transform.position = blocPos1;
            InstanciateWall();
                
            Vector3 blocPos3 = new Vector3(j, 0, -toInstanciate.matrixLength*10);
            transform.position = blocPos3;
            InstanciateWall();
        }
    }
